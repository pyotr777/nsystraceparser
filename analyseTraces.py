...
def getCleanTimes(df, debug=False):
    # Time without cuda... events
    gpu = df[df['GPU side'] == True].copy()
    noncompute_events = selectEventsByName(gpu, [
        "cudaStreamSynchronize", "cudaMemcpyAsync", "cudaEventQuery", 'cudaStream',
        'cudaEvent', 'cudaBindTexture', "cudaStreamWaitEvent", "cudaMemcpy", "cudaMemset",
        "cudaDeviceSynchronize", 'cudaUnbindTexture', 'cudaEventCreateWithFlags',
        'cudaHostAlloc', 'cudaMalloc', "cudaFree", "cudaLaunchKernel",
        "cudaThreadSynchronize", "gen_sequenced", "generate_seed_pseudo", "CUPTI"
    ])
    clean_gpu = gpu[~gpu['corrID'].isin(noncompute_events['corrID'].unique())]
    clean_gpu = clean_gpu[clean_gpu['name'].notna()]

    cpu = df[df['GPU side'] == False].copy()
    noncompute_events = selectEventsByName(cpu, [
        "cudaStreamSynchronize", "cudaMemcpyAsync", "cudaEventQuery", 'cudaStream',
        'cudaEvent', 'cudaBindTexture', "cudaStreamWaitEvent", "cudaMemcpy", "cudaMemset",
        "cudaDeviceSynchronize", 'cudaUnbindTexture', 'cudaEventCreateWithFlags',
        'cudaHostAlloc', 'cudaMalloc', "cudaFree", "cudaLaunchKernel",
    ...
    clean_cpu = cpu[~cpu['corrID'].isin(noncompute_events['corrID'].unique())]
    clean_cpu = clean_cpu[clean_cpu['name'].notna()]

    if debug:
        print("Compute CPU events: {}".format(clean_cpu.shape[0]))
        print("Compute GPU events: {}".format(clean_gpu.shape[0]))

    return clean_cpu, clean_gpu
...
# filter - additional plot with only events wuth names matching the filter
def plotArea(df, series='name', title=None, limit=20, agg='sum', yticks=None,
             values='duration', av_func='median', units="", filters=None):
    df_ = None
    if 'iteration' in df.columns:
        df_ = df.groupby([series, 'param', 'iteration'
                          ]).agg(agg).groupby([series, 'param'
                                               ]).agg(av_func).reset_index(drop=False)
        # title += "\nIteration time"
    else:
        df_ = df.groupby([series, 'param']).agg(agg).reset_index(drop=False)
    ...
    if filters is None:
        ...
        plots = len(filters) + 1
        fig, axs = plt.subplots(plots, 1, figsize=(10, 12), dpi=144)
        ax = axs[0]
    n = df_.shape[0]
    ...
    if filters is not None:
        for i, filter in enumerate(filters):
            ...
            df_ = df_.filter(like=filter, axis=0)
            # df_.loc[:, 'total'] = df_.sum(axis=1)
            # df_ = df_.sort_values(['total'])
            # df_.drop(['total'], axis=1, inplace=True)
            df_.columns = df_.columns.get_level_values(1)
            ...
            ax.grid(ls=':', lw=0.3, which='minor')
            ax.set_ylabel("{} of {} ({})".format(agg, values, units))
            ax.set_title("filter={}".format(filter))
            # Reverse legend order
            handles, labels = ax.get_legend_handles_labels()
            handles = handles[::-1][:limit]
            labels = labels[::-1][:limit]
...
def main():
    ...
    parser.add_argument(
        "--events", default=None, required=False, nargs='*', help=
        "Event name patterns. Multiple space-separated values possible." \
        " Have priority over event_filters in configuration file."
    )
    ...
    if args.conf is not None:
        ...
        if os.path.isfile(args.conf):
            ...
            event_filters = conf["event_filters"]
    ...
    if nvtx.shape[0] > 0:
        ...
        nvtx = nvtx[['duration', 'NVTX',
                     'param']].pivot_table(index='param', columns='NVTX',
            ...
            for i in range(0, results.shape[0], slicesize):
                ...
                print(
                    "\rParsing range {}-{}M lines of {:.1f}M ({:.1f})s.".format(
                        ...
                        results.shape[0] / 1000000, time2 - time1), end='')
    ...
    if 'iteration' in longtimes.columns:
        ...
        ax = axs[0]
        iteration_variability.plot(y=['mean', 'delta'], marker='o', ms=4, mfc='w',
                                   mew=0.5, ax=ax)
        ...
        ax = axs[1]
        iteration_variability.plot(y='variability', marker='o', ms=4, mfc='w', mew=0.5,
                                   ax=ax)
        ax.xaxis.set_major_locator(MultipleLocator(10))
        ax.yaxis.set_minor_locator(AutoMinorLocator(5))
        ax.grid(ls=':', lw=0.7)
    ...
    clean_cpu, clean_gpu = getCleanTimes(longtimes)
    ...
    if memusage > 100000000:
        ...
        axs[0].plot(clean_cpu.param, clean_cpu.duration, marker='o', ms=3, mew=0,
        ...
        axs[0].grid(ls=':')
        axs[1].grid(ls=':')
        figfile = "CPUcomputeKernelDurations.png"
        figfile = os.path.join(targetdir, figfile)
        plt.savefig(figfile, bbox_inches="tight")
        print("Saved", figfile)
    ...
    columns = [
        c for c in ['name', 'duration', 'param', 'iteration'] if c in clean_cpu.columns
    ]
    df_ = clean_cpu[columns].copy()
    df_['duration'] = df_['duration'] * 1000.
    plotArea(df_, series='name',
             title="Compute kernel calls, median time\n{}".format(logdir), yticks=(4, 20),
             units="ms")
    ...
    df_ = clean_cpu[columns].copy()
    plotArea(df_, series='name', title="Compute kernel calls number\n{}".format(logdir),
    ...
    try:
        ...
        if 'iteration' in df_.columns:
            df_ = df_.groupby(['param', 'name',
                               'iteration']).agg('sum').reset_index(drop=False)
            title = "{} time per iteration for\n{}".format(event, logdir)
        else:
            df_ = df_.groupby(['param', 'name']).agg('sum').reset_index(drop=False)
            title = "{} time for\n{}".format(event, logdir)
    ...
    # Data Copy Time (Memcpy) per iteration
    memcopyagg = None
    if 'iteration' in longtimes.columns:
    ...
    if callsandgapsGPU is not None:
    ...
    if memcopyagg is not None:
        ...
        CPUGPU = CPUGPU.merge(df_, on="batch")
        print("CPUGPU with memcpy:\n", CPUGPU.head())

    # # Aggregate clean CPU time
    clean_cpu_agg = None
    if 'iteration' in clean_cpu.columns:
        df_ = clean_cpu[['duration', 'param', 'iteration']].copy()
        clean_cpu_agg = df_.groupby(['param', 'iteration'],
                                    as_index=False).agg('sum').groupby(
                                        ['param'], as_index=False).agg('median').drop(
                                            ['iteration'], axis=1).rename(
                                                {
                                                    'param': 'batch',
                                                    'duration': 'clean CPU'
                                                }, axis=1)
    else:
        clean_cpu_agg = clean_cpu[['duration',
                                   'param']].groupby(['param'],
    ...
    # # Aggregate clean GPU time
    clean_gpu_agg = None
    if 'iteration' in clean_gpu.columns:
        df_ = clean_gpu[['duration', 'param', 'iteration']].copy()
        clean_gpu_agg = df_.groupby(['param', 'iteration'],
                                    as_index=False).agg('sum').groupby(
                                        ['param'], as_index=False).agg('median').drop(
                                            ['iteration'], axis=1).rename(
                                                {
        ...
        clean_gpu_agg = clean_gpu[['duration',
                                   'param']].groupby(['param'],
                                                     as_index=False).agg('sum').rename(
                                                         {
                                                             'param': 'batch',
    ...
    print('Clean CPU agg time\n', clean_cpu_agg.head())
    print('Clean GPU agg time\n', clean_gpu_agg.head())
    if CPUGPU is not None:
        CPUGPU = CPUGPU.merge(clean_cpu_agg, on="batch")
        CPUGPU = CPUGPU.merge(clean_gpu_agg, on="batch").set_index('batch', drop=True)
    else:
        CPUGPU = clean_cpu_agg.merge(clean_gpu_agg,
                                     on="batch").set_index('batch', drop=True)
    ...
    nogapscolumns = [c for c in df.columns if 'gaps' not in c and 'batch' not in c]
    ...
    for lim in lims:
        ...
        if dataio is not None:
            dataio.loc[:lim, :].plot(y='mean_read_time', use_index=True, ms=4,
                                     mfc='white', marker='o', ax=ax)
        if dnnmark is not None:
            dnnmark[dnnmark['batch'] <= lim].plot(x='batch', ms=4, mfc='white',
                                                  marker='o', ax=ax)
        ...
        ax = axs[1]
        # Plot iteration time (or sum of all times )
        if 'batch' in df.columns:
            df_ = df.set_index('batch', drop=True)
        else:
            df_ = df
        df_.loc[:lim, nogapscolumns].plot(ms=4, lw=1, mfc='white', marker='o', ax=ax)
        if len(withgapscolumns) > 0:
            df.loc[:lim, withgapscolumns].plot(ms=2.5, lw=0.6, mfc='white', mew=0.5,
                                               marker='o', ax=ax)
        if nvtx.shape[0] > 0:
            nvtx.loc[:lim, :].plot(lw=1, marker='o', ms=4, mfc='white', mew=0.8, ls='--',
                                   ax=ax)
        if dataio is not None:
            dataio.loc[:lim, :].plot(y='mean_read_time', use_index=True, ls=":", lw=1,
                                     ax=ax)
        if dnnmark is not None:
            dnnmark[dnnmark['batch'] <= lim].plot(x='batch', ms=3, mfc='white',
                                                  marker='o', ls=":", lw=1, mew=0.5,
                                                  ax=ax)
    ...
    if 'log_time' in df.columns:
        ...
        for i, col in enumerate(CPUcolumns):
            ...
            ax = axs[i]
            df.loc[:maxmbs].plot(x=sum_column, y='log_time', marker='o', ms=5, mfc='w',
                                 ax=ax)
        ...
        f = lrmodel.predict(X)
        fig, ax = plt.subplots(figsize=(8, 5), dpi=140)
        ax.plot(X, Y, label='Itertime(CPU+memcopy)', marker='o', ms=4, mfc='white',
                mew=0.7)
        ax.plot(X, f, label="LR", ls=':')
        ...
        fig.suptitle(
            "Itertime(CPU+memcopy)\nLR = $a{:.2f} {:+.2f}$".format(
                lrmodel.coef_[0], lrmodel.intercept_), fontsize=14, y=1.02)
        ax.set_title(logdir, fontsize=10)
        ax.grid(ls=':', lw=0.5)
